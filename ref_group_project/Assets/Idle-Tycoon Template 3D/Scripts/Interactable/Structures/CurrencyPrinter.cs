...
{
    // Currency Printer is a structure which creates a collectable every print cycle
    // The collectable can be picked up to recieve currency
    public class CurrencyPrinter : Structure
    {
        [Header("Printer")]
        [Tooltip("Collectable spawned by the printer")]
        public CurrencyCollectable currencyObject; // Collectable Spawned 
        [Tooltip("Location at which the Printer will place the collectables when spawned")]
        public Vector3 printLocation;

        /// <summary>
        /// Gets the Spawn Location for the collectable to be placed at.
        /// </summary>
        public Vector3 spawnLocation => this.transform.position + printLocation;
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            Gizmos.DrawSphere(spawnLocation, 0.1f);
        }
